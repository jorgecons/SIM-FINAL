 ...
 * @author jorge
 */
public class GestorPoliticaActual {
    ...
    public GestorPoliticaActual(double tiempoSimulacion) {
    ...
    public void simular() {
        ...
        llenado = vectorEstado[1].getTiemposFinLlenado().size();
    ...
    private void siguienteLinea() {
        tiempoActual = vectorEstado[1].getMenorTiempo();
        if(tiempoSimulacion<tiempoActual){
            return;
        }
        VectorEstado vec = new VectorEstado();
        ...
        if (vec.getEventoActual() == 1) {
            vec.llegadaCliente();
            vec.llenarFormulario();
        }
        if (vec.getEventoActual() == 2) {
            int numCliente = vectorEstado[1].getNumCliente();
            vec.asignarAEmpleado(numCliente);
    ...
    public String toString() {
        return "GestorPoliticaActual{" + "vectorMostrar=" + vectorMostrar.toString() + '}';
    }
    ...
    private void primeraLinea() {
        tiempoActual = vectorEstado[0].getMenorTiempo();
        VectorEstado vec = new VectorEstado();
        copiar(vec, 0);
    ...
    private void copiar(VectorEstado vec, int pos) {
        ...
        //tiempos fin de llenado
        ArrayList tiemposAux = new ArrayList<>();
        for (int i = 0; i < vectorEstado[pos].getTiemposFinLlenado().size(); i++) {
            tiemposAux.add(vectorEstado[pos].getTiemposFinLlenado().get(i));
        }
        vec.setTiemposFinLlenado(tiemposAux);
